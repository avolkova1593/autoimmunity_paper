...
###Script for creating PCOA plots from 16S relative abundance tables
...
my_file <- read.table("ml_ready_all_16s_6.txt", as.is =T,header=T, sep="\t")
...
colors <- c("#003859","#88fa31","#6300b3","#ecff35","#000797","#8bd000","#0138cd","#00b116","#f723dc","#00dc63",
            ...
            "#4b3100")
study_colors <- c(sample(colors)[1:36])
##Remove the samples with all zeros
my_file <- my_file[rowSums(my_file[, -c(1:29)] > 0) != 0, ]
###Compute PCoA
distance_bray <- vegdist(my_file[,-c(1:29)], method = "bray")
...
pcoa_df$Region16S <- my_file$Region16S
pcoa_df$Platform <- my_file$Platform
...
permanova_region16s <- adonis(distance_bray ~ Region16S, data=pcoa_df)$aov.tab$`Pr(>F)`[1]
permanova_platform <- adonis(distance_bray ~ Platform, data=pcoa_df)$aov.tab$`Pr(>F)`[1]
...
ggplot(pcoa_df,aes(V1,V2,colour = Disease)) + 
  geom_point(size=1)+
  scale_colour_manual(values=c("red", "magenta","forestgreen",
                               "steelblue3","turquoise4", "royalblue3",
                              "pink3", "purple3","cyan2", "orange3",
                               "yellow3", "black", "coral2",
                                "deepskyblue2", "hotpink4", "green3"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
  guides(colour = guide_legend(override.aes = list(size=6)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        ...
        text = element_text(size=14),
        legend.title=element_blank())
###PCoA colored by sequencing platform
ggplot(pcoa_df,aes(V1,V2,colour = Platform)) + 
  geom_point(size=1)+
  scale_colour_manual(values=c("deeppink2", "royalblue2", "green3"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
  guides(colour = guide_legend(override.aes = list(size=6)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        ...
        text = element_text(size=14),
        legend.title=element_blank())
###PCoA colored by 16S rRNA region
ggplot(pcoa_df,aes(V1,V2,colour = Region16S)) + 
  geom_point(size=1)+
  scale_colour_manual(values=c("turquoise4", "red3", "steelblue2", 
                               "springgreen1","purple3",
                               "magenta3","blue3","hotpink2",
                               "darkorange3","forestgreen"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
  guides(colour = guide_legend(override.aes = list(size=6)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        ...
        text = element_text(size=14),
        legend.title=element_blank())
###PCoA colored by Country of origin
ggplot(pcoa_df,aes(V1,V2,colour = Location)) + 
  geom_point(size=1)+   scale_color_manual(values = c("turquoise4", "red2",  "brown",
                                                      "purple3","green3","steelblue2",
                                                      "magenta3",
                                                      "plum3","deeppink4",  "rosybrown4",
                                                      "red3", "royalblue2", "olivedrab3",
                                                      "navyblue", "maroon",  "gold3"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
  guides(colour = guide_legend(override.aes = list(size=6)))+
  theme_bw()+
...
ggplot(pcoa_df,aes(V1,V2,colour = Antibiotics)) + 
  geom_point(size=1)+   scale_color_manual(values = c("blue", "red",  "brown",
                                                      "purple","green","gold",
                                                      "magenta","grey80"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
...
ggplot(pcoa_df,aes(V1,V2,colour = DNAExtractionkit)) + 
  geom_point(size=1)+   
  scale_color_manual(values = c("turquoise4", "red2",  "brown",
                                "purple3","green3","steelblue2",
                                 "magenta3","deeppink4","gold3",
                                 "red3", "royalblue2", "grey80","olivedrab3",
                                  "navyblue", "maroon",  "blue"),
                     limits = c("Custom","Enzymatic lysis method",
                                "International Human Microbiome Standards",
                                "Longsee Stool DNA kit",
                                "MasterPure Complete DNA and RNA Purification kit",
                                "MoBio Power Soil DNA Isolation kit", 
                                "Norgen Stool DNA Isolation kit",
                                "Phenol chloroform method","QIAamp DNA Stool kit",
                                "QIAGEN DNeasy Power Soil DNA Isolation kit",
                                "Zymo Research kit","not_available"))+
  labs(title="", x=paste0("PC1 (",round(my_pcoa$values$Relative_eig[1]*100, digits=3),"%)"), 
       y=paste0("PC2 (",round(my_pcoa$values$Relative_eig[2]*100, digits=3),"%)"))+
  guides(colour = guide_legend(override.aes = list(size=6)))+
  theme_bw()+
